<template>
  <div>
    ...
    <div name="calc" class="calc" style="text-align: right; padding: 10px 0">
      <span>
        Total units of passed and not-released Major Elective Courses:</span
      >
      <input
        type="text"
        class="iptText"
...
export default {
  ...
  computed: {
    ...
    textData() {
      return this.txData.filter(x => x.categoryCode === 'Major Elective Courses' && x.part === 'A');
    },
    partTitle() {
      if (this.textData && this.textData.length > 0) {
        return this.textData[0].categoryName;
      } else {
        return '';
      }
    },
