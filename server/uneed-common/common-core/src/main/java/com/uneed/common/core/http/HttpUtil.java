...
import static com.uneed.common.core.lang.ObjectUtil.notEqual;
...
public class HttpUtil {
    ...
    public static JSONObject doPostByJson(String reqUrl, JSONObject param, Integer timeout) throws IOException {
        ...
        try {
            ...
            if (sb.length() > 0) {
                returnObj = JSONObject.parseObject(sb.toString().replaceAll("\n", "").replaceAll("null", "\"null\""));
            }
            // 断开连接
        } catch (Exception e) {
    ...
    private static boolean ipNotEmpty(String ip) {
        return StringUtil.isNotEmpty(ip) && notEqual("unknown", StringUtil.toLowerCase(ip));
    }
}
